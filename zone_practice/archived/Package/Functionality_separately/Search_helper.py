...
def addTender(info, date, all_tenders, all_announcements, different_statuses):
    for rec in info['items']:
        if 'number_anno' in rec and rec['number_anno'] in all_announcements:
            break
        ...
        if 'number_anno' in rec:
            all_announcements.append(rec['number_anno'])
            num_anno = rec['number_anno']
        if 'name_ru' in rec:
            name_ru = rec['name_ru']
        ...
        all_tenders.append(
            make_tender(num_anno, name_ru, publish, end_date, total_sum, status))


def make_tender(num_anno, name_ru, publish, end_date, total_sum, status):
    tender = {'anno': num_anno,
              'name': name_ru,
              'publish': publish,
              'date': end_date,
              'sum': total_sum,
