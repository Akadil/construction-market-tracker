...
# Output: 1 if later or same day, 0 if earlier
...
def addTender(info, mydate, all_tenders, all_announcements, different_statuses):
    for rec in info['items']:
        if 'number_anno' in rec and rec['id'] in all_announcements:
            break
        if 'total_sum' in rec and rec['total_sum'] < 200000000:
            continue
        if 'publish_date' in rec and (isLaterDate(mydate, rec['publish_date'])):
            continue
        if 'ref_buy_status_id' in rec and not (rec['ref_buy_status_id'] in different_statuses):
            continue
        if 'name_ru' in rec and ("слуг" in rec['name_ru'] or
                                 "риобретение" in rec['name_ru'] or
                                 "разработ" in rec['name_ru'] or
                                 "надзор" in rec['name_ru'] or
                                 "внутридвор" in rec['name_ru'] or
                                 "аула" in rec['name_ru'] or
                                 "уборка" in rec['name_ru'] or
                                 "благоустройс" in rec['name_ru'] or
                                 "Благоустройс" in rec['name_ru'] or
                                 "азвитие" in rec['name_ru'] or
                                 "сатып алу" in rec['name_ru'] or
                                 "нерготи" in rec['name_ru']):
            continue
...
def main():
    ...
    try:
        ...
        short = cursor.execute("select date from metadata")
        my_date = short.fetchall()[0][0]

        cursor.execute("delete from metadata")

        cursor.execute(f'insert into metadata values("{str(date.today())}")')
    ...
    addTender(info1, my_date, all_tenders, all_announcements, different_statuses)
    ...
    addTender(info2, my_date, all_tenders, all_announcements, different_statuses)
